...
- **State** (`state.json`). Per session id: `agreed`, the normalised hash at which every enrolled partition last held the same state. Per partition: `seen`, every id the tool has observed there at any scan or created there, forgotten only when no partition holds the record any more; `placing`, creates journalled by a run that did not finish and not proven to have completed; `placed`, the hash the tool last put there for an id, remembered while that copy is untouched and not yet agreed. Per app data root: the login last seen there and when the tool first saw it (R9). Beside it, `placing.log` is the create journal of R10: one line before each create, one after it completed.
...
**R7 Lost records.** An id absent from a partition that is in that partition's `seen` or `placing`, with no tombstone anywhere, was removed there without a tombstone (F3 mid-delete, or a failed tombstone write). It is never recreated there. What a run saw at its first scan is saved before its first write, and what it created counts as seen even if the final scan no longer shows it, so a record removed in the middle of a run is still known to have been there. It is reported, and `--recreate ID` lifts the hold for one session. An id gone from every partition is forgotten.

A create that the journal proves completed counts as seen, whatever happened to the run that made it. A create the journal only shows as intended may never have happened: it is held back for one run, reported as held, and then retried. By then a delete marker has turned up if the app removed the record.

**R8 Nothing unique is destroyed.** A replaced copy is kept in `kept/<run>/<account>_<org>/` unless it is still as synced and the winner's activity is strictly greater. A retired record, temp file or tombstone is always kept. Kept files never overwrite each other, and the report names where each went. Kept runs are pruned after a run with no failures: older than 30 days, and oldest first above 500 MB. The run that just finished is never pruned.
...
**R10 Crash safety.** The journal gets a line just before each create and another once it has completed, and stop signals are held back between a create and its second line so the two cannot be split. The journal is cleared after a run has saved its state; a run that did not finish leaves it for the next run (R7). Results are saved after the writes. Every file is staged under a name that starts with `.sync-` and renamed or linked into place, and a staged file is discarded on any exit between the two. Stale `.sync-` files are swept from the partitions and from the tool's own folders. SIGTERM unwinds through cleanup. Commands that edit the sync history take the run lock. A state file that cannot be trusted aborts the run; `--reset-state` keeps the old file and starts again from first contact.
...
- **A create and its proof.** Stop signals cannot land between a create and its journal line, but a kill that cannot be caught, or a power loss, can. The create is then retried after one held run. It goes wrong only if the record was also removed without a delete marker before that.
