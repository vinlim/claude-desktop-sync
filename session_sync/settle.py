...
def settle(state: SyncState, snapshots: List[Snapshot]) -> SyncState:
    """state.seen already holds what the run saw at its first scan and what it created: both are
    written down as they happen. Presence is everything the tool has observed in a partition or
    created there, and an id leaves it only when no partition holds the record any more. A record
    that was there and is gone again was removed by the app, and R7 must know it was there."""
