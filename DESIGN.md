...
- **A retire that stops halfway.** A record with a leftover temp file is retired temp file first, because a temp file left alone would be promoted by the app. If the record then fails its guard, the temp file is already in `kept/` and the report does not name it. That takes an app save cut short and a change to the record in the next few milliseconds. The next run decides again from what it finds.
