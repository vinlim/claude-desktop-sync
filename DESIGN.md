...
Presence is written down as it arises. What a run saw at its first scan is saved before its first write. Each create is saved the moment it completes, and a create that cannot be saved is undone. So a record removed in the middle of a run, or after a run that did not finish, is still known to have been there.
